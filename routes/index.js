...
router.get('/', function(req, res, next) {
	...
	var products = Product.find(function(err,docs){
		...
		res.render('shop/index', { title: 'Awesome Clothes E-commerce', products: productChunks,active: active, successMsg: successMsg,noMessage: !successMsg });
	});
});
...
router.get('/findby/', function(req,res,next){
	...
	var products = Product.find({ majorTitle: value }, function(err,docs){
		...
		res.render('shop/index', { title: 'Shopping-Cart', products: productChunks,active: active, successMsg: successMsg,noMessage: !successMsg });
	});
...
router.post('/item', function(req,res,next){
	...
	var product = Product.findOne({productID: req.body.id}, function(err,docs){
...
router.get('/shoppingcart', function(req,res,next){
	...
	res.render('shop/shoppingcartv2', {products: cart.generateArray(), totalPrice: cart.totalPrice, qty: cart.totalQty});
});
...
router.get('/checkout',isLoggedIn, function(req,res,next){
	...
	res.render('shop/checkoutv2', {total: cart.totalPrice, qty:cart.totalQty, items:cart.items, errMsg: errMsg, noErrors: !errMsg});
});
